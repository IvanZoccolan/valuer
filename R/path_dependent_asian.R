...
#'   \item{\code{cash_flow_times}}{retuns a \code{\link{timeDate}} object with the possible cash flow times. It returns the delivery time of the arithmetic asian option}
...
path_dependent_asian <- R6Class("path_dependent_asian", inherit = path_dependent,
  public = list(
    ...
    max_number_cfs = function() {1L},

    cash_flow_times = function() {private$delivery_time},

    cash_flows = function(spot_values){

      mean_spot <- mean(spot_values)
      cash_flows$new(private$delivery_time, private$the_payoff[mean_spot])
